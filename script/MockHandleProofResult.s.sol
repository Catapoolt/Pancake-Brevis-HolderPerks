...
contract MockHandleProofResult is Script {
    ...
    function run() public {
        ...
        // bytes32 idBytes = PoolId.unwrap(key.toId());
        // console.logBytes32(bytes32(idBytes));

        bytes memory output = abi.encodePacked(SHARE, POOL_ID, LIQ_PROVIDER, PERIOD_ID);

        vm.broadcast();
        HolderPerks(HOOK).handleProofResultMock(bytes32(0), output);
    }
}